...
const sendHelpMenu = (message: Message, instance: YolocatCommands) => {
...
module.exports = {
  ...
  init: (client: Client, instance: YolocatCommands) => {
  ...
  callback: (options: ICallbackObject) => {
    ...
    if (guild && !guild.me?.permissions.has('SEND_MESSAGES')) {
      console.warn(
        `YolocatCommands > Could not send message due to no permissions in channel for ${guild.name}`
      )
      return
    }
