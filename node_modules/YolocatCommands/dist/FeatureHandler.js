...
class FeatureHandler {
    ...
    setup = async (dir, typeScript) => {
        ...
        if (amount > 0) {
            console.log(`YolocatCommands > Loading ${amount} listener${amount === 1 ? '' : 's'}...`);
            ...
            console.log(`YolocatCommands > Loaded ${amount} listener${amount === 1 ? '' : 's'}.`);
        }
    };
    registerFeature = (file, fileName) => {
        ...
        if (config) {
            ...
            if (missing.length && this._instance.showWarns) {
                console.warn(`YolocatCommands > Feature "${fileName}" has a config file that doesn't contain the following properties: ${missing}`);
            }
        }
        else if (this._instance.showWarns) {
            console.warn(`YolocatCommands > Feature "${fileName}" does not export a config object.`);
        }
        ...
        if (config && config.loadDBFirst === true) {
            console.warn(`YolocatCommands > config.loadDBFirst in features is no longer required. MongoDB is now connected to before any features or commands are loaded.`);
        }
